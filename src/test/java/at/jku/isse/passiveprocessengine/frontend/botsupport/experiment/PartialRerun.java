...
class PartialRerun {
	...
	@Test 
	void runEvaluationAndLog2Cgemma2() throws Exception {
	...
	@Test 
	void runEvaluationAndLogA2codegeeX4() throws Exception {
		openAImessages.clear(); 
		ollamaAImessages.clear();
		schemaGen = new HumanReadableSchemaExtractor(schemaReg);
		repairer = new IterativeRepairer(provider);		
		bot = new OllamaAI("http://10.78.115.48:11434/api/chat", "codegeex4:latest", provider);
		String modelForFilePath = "codegeex4";
		var groundTruth = a2;
		var prompt = """ 
	...
	private EvalResult runPartialRound(ConstraintGroundTruth groundTruth, String initialPrompt, String initialAnswer) throws Exception {
		
		var result = new EvalResult(groundTruth.getTaskId());
		
		//generate prompt
		var prompt = initialPrompt;
		var answer1 = initialAnswer;
	...
	private String getResponse(String prompt, AbstractBot bot) throws Exception {
		...
		//	var req = new OllamaAI.ChatRequest(prompt, null)
			...
			ollamaAImessages.add(new at.jku.isse.passiveprocessengine.frontend.botsupport.ollama.OllamaAI.Message("user", prompt));
			var req = new ChatRequest(ollamaAI.getModel(), ollamaAImessages);
