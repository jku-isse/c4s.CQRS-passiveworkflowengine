...
public class TestGenerateOCL {
	...
	@Test
	void unloadAndLoad() throws IOException {
		var utils = new OllamaUtils(bot);
		// unload
//		var resp = utils.unloadModel("");
//		assertTrue(resp.isDone());
		// load
//		var resp2 = utils.loadModel(model);
...
//    	```
	...
	void testGenerateReqWithFindingsNotVerified() throws Exception {
		var prompt = compilePrompt(schemaStepAndReviewAugmented, 
    	...
    	var nonAugmentedcodestral = """
    			 self.in_SWRequirementsColl->select(req | req.oclIsKindOf(Requirement))->forAll(
    			   req : Requirement |
    			      req.state <> 'Verified' implies
    			         not self.out_ReviewFinding->exists(finding | finding.oclIsKindOf(Review) and finding.needRereview = true and finding.in_Review->includes(req))
    			)
    			""";
    	var augmentedcodestral = """
    			self.out_ReviewFinding->exists(rf | rf.oclIsKindOf(Reviewfinding) and not self.in_SWRequirementsColl->exists(r | r.oclIsKindOf(Requirement) and r.state = 'Verified'))
    			""";
	}
	...
	void testGenerateBugsLinkToRequirements() throws Exception {
    	...
    	var codestral = """
    			 self.out_Bugs->forAll(bug | bug.affectedByItems->exists(req | req.oclIsKindOf(Requirement) and req.state <> 'Released'))
    			""";
	...
	public static String extractAnswer(OllamaAI.ChatResponse response) {		
	...
	static String schemaStepAndReviewAugmented = 
			"Generic object type azure_workitem contains following properties:\r\n"
			+ " activatedBy of type  user\r\n"
			+ " activatedDate of type  STRING\r\n"
			+ " affectedByItems of multiple  azure_workitem\r\n"
			...
			+ "\r\n"
			+ "Generic object type ProcessStep contains following properties:\r\n"
			+ " in_Review of multiple  Review\r\n"
			+ " in_SWRequirementsColl of multiple  Requirement\r\n"
			+ " out_ReviewFinding of multiple Reviewfinding\r\n"
			+ "";
