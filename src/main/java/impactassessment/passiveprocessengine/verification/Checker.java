...
public class Checker {
    ...
    private Report.Warning[] checkUnconnectedDecisionNode(AbstractWorkflowDefinition workflow) {
        if (workflow.getDecisionNodeDefinitions().size() > graph.getNodes().stream().filter(n -> n.getType().equals(DND)).count()) {
            return new Report.Warning[]{ new Report.Warning("There exist unconnected DecisionNodeDefinition(s)!", "")};
        }
        return null;
    }

    private Report.Warning[] checkUnconnectedTaskDefinition(AbstractWorkflowDefinition workflow) {
        if (workflow.getWorkflowTaskDefinitions().size() > graph.getNodes().stream().filter(n -> n.getType().equals(TD)).count()) {
            return new Report.Warning[]{ new Report.Warning("There exist unconnected TaskDefinition(s)!", "")};
        }
        return null;
    }
